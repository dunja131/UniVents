...
class RsvpService {
  static const String _baseUrl =
      'http://localhost:8080/api/rsvps'; //backend address

  final UserService
  _userService; //userService provides us with the logged in user's details (token, ID)
  ...
  //Get the RSVP ID and Event ID for the particular user
  Future<int?> getRsvpId({required int eventId}) async {
    final String? userId = _userService.userId; //gets the logged in user's ID

    final response = await http.get(
      //this requests backend for all RSVPs associated with user
      Uri.parse('http://localhost:8080/api/rsvps/user/$userId'),
    ...
    if (response.statusCode == 200) {
      final List data = jsonDecode(
        response.body,
      ); //converts the JSON response to a Dart List

      // loop through each rsvp and find the matching one
      for (var rsvp in data) {
        if (rsvp['event']['eventId'] == eventId) {
          return rsvp['rsvpId'];
        }
      }
  ...
  // Delete rsvp on ladning page as in the user has rsvp'd but wants to now undo it
  Future<void> deleteRsvp({required int rsvpId}) async {
    final response = await http.delete(
      Uri.parse('http://localhost:8080/api/rsvps/$rsvpId'),
      headers: {'Authorization': _userService.authHeader},
    );

    if (response.statusCode != 204) {
      throw Exception('Failed to delete RSVP (${response.statusCode})');
    }
  }
}
